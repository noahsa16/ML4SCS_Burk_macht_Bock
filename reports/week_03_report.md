...
- **Live handwriting preview fixed**: the canvas was rendering nothing the entire week. Diagnosed: every Moleskine `PEN_DOWN` carries `x=-1, y=-1` (sentinel — the actual coordinate arrives with the first `PEN_MOVE`). The server's preview filter dropped those `-1,-1` rows, so the client only ever saw `PEN_MOVE` events with no preceding `PEN_DOWN`, and the renderer's `if (inStroke)` guard meant nothing was ever drawn. Fixed the renderer to start a new stroke at the first `PEN_MOVE`. Also raised the rolling preview window from 200 to 2500 dots, hardened the dedup key, and added a dot-fallback so even if path rendering fails the user sees points.
...
- **Pen firmware update**: updating the Moleskine firmware closed the long-standing `PEN_DOWN` / `PEN_UP` mismatch. Session S016 (pre-update) had 81 `PEN_DOWN` events but only 78 `PEN_UP` events; post-update session S018 has 19/19, fully matched.
