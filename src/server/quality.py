...
from .issues import (
    ...
    _WATCH_HZ_MAX, _WATCH_HZ_MIN, watch_in_range, watch_target_hz,
...
def _session_facts(row: dict[str, str]) -> dict[str, Any]:
    ...
    # Why: Watch kann 50 oder 100 Hz fahren; die erwartete Sample-Zahl muss
    # vom beobachteten Target abhaengen, nicht von einer hart codierten 50.
    # Sonst zaehlt eine 100-Hz-Session bei 200% der Erwartung und low_watch_
    # coverage uebersieht reale Drops.
    _target_hz = watch_target_hz(watch_est_hz)
    expected_watch_samples = int(expected_duration * _target_hz) if expected_duration else None
...
def _build_issues(facts: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    hz = w["estimated_hz"]
    if hz is not None and not watch_in_range(hz):
        target = watch_target_hz(hz)
        out.append(_make_issue(
            "watch_rate_out_of_range",
            observed=f"{hz:.1f} Hz (target ≈ {target:.0f} Hz)",
        ))
...
def _session_report_markdown(report: dict[str, Any]) -> str:
    ...
    L.append("")
    L.append("Apple Watch IMU (accelerometer + gyroscope), 50 or 100 Hz "
             "(per-session Nearest-Match) via WatchConnectivity → iPhone bridge → HTTP POST.")
    L.append("")
    L.append("| Metric | Value | Target / threshold |")
    L.append("|---|---|---|")
    L.append(f"| Samples | {fmt_num(w['rows'])} | — |")
    est_hz = w.get('estimated_hz')
    target_hz = watch_target_hz(est_hz) if isinstance(est_hz, (int, float)) else report["target_watch_hz"]
    hz_compliance = ""
    if isinstance(est_hz, (int, float)):
        hz_compliance = "✓ in band" if watch_in_range(est_hz) else "✗ out of band"
    L.append(f"| Estimated rate | **{est_hz or '—'} Hz** {hz_compliance} | "
             f"≈ {target_hz:.0f} Hz ±{int(0.20*100)} % |")
