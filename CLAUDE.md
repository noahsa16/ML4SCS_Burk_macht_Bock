...
Moleskine Smart Pen (BLE)
  ...
  → data/raw/pen/{session}_pen.csv
       ...
       scripts/pipeline/train_noah_personal.py    (Personal-Modell auf S032+
                                      S033 ohne Z-Score → rf_noah)
       scripts/pipeline/train_rf_all_live.py      (Deployment-Variant des
...
inference.py       LiveInference singleton: rolling watch-sample buffer,
                   ...
                   **Kausaler HMM-Live-Filter (seit 2026-06-24):** die rohe
                   ...
                   loso_oof.csv via `scripts/pipeline/export_hmm_live.py`; ~16 s
...
- `scripts/pipeline/train_noah_personal.py` — Personal-Modell für die Focus-Tracker-App.
...
- `scripts/pipeline/train_rf_all_live.py` — Deployment-Variante des
...
- `scripts/pipeline/export_hmm_live.py` — schreibt die deploybaren Live-HMM-Parameter
...
- `scripts/ml/hmm_postprocess_loso.py` — HMM-Treiber auf `models/loso_oof.csv`,
  ...
  *Rolling-Mean* hebt nichts, ein *HMM* schon. **Seit 2026-06-24 LIVE deployed**
  (`OnlineForwardFilter`; Parameter via `scripts/pipeline/export_hmm_live.py` →
...
- `scripts/` is split by consequence, not by topic: `scripts/pipeline/`
  writes artefacts the running server loads (`rf_noah`, `rf_all_live`,
  `hmm_live.json`), everything under `scripts/ml/` is the research
  record. `scripts/README.md` maps all of it. Do not rename files under
  `scripts/ml/` or `scripts/plots/` — external runners (RunPod pods,
  notebooks) clone this repo and call those paths directly, so a rename
  fails silently until the next training run.
- Never commit proband data. Our own recordings (`data/raw/`), the Swiss
  partner's (`data/foreign_data/`) and every derived bundle
  (`sweep_data.zip`, `foreign_adapter/output/`) are gitignored. Before
  any commit that touches `data/`, check `git diff --cached --name-only`
  — on 2026-08-08 already-staged foreign recordings rode along with an
  unrelated commit into both remotes and had to be removed with
  `git filter-repo`.
